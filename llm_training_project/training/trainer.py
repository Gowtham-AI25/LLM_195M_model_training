...
def train_on_shard(
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        # Decide if we update: protects against the "Double-Tap" spike
        ...
        if not should_update:
            ...
            # 3. Optimization Step preparation
            scaler.unscale_(optimizer)
            
            # --- SCALE FIX FOR PARTIAL BATCHES ---
            ...
            # ---------------------------------------------------------
            # 4. FULL LOGGING (WandB, Tensor Diagnostics, Metrics)
            # ---------------------------------------------------------
            if rank == 0:
                ...
                # A. Console Print
                print(f"Step {global_step} | Loss: {avg_step_loss:.4f} | Perplexity: {perplexity:.2f} | LR: {current_lr:.6f}", flush=True)

                if writer is not None:
                    # B. Tensor/Histogram Logging (Every 25 global steps)
                    if is_final_batch:
                ...
                if wandb.run is not None: 
                    wandb.log({
                        ...
                        "performance/learning_rate": current_lr,
        ...
        # Global stats tracking (per micro-batch)
